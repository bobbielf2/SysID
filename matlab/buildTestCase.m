function model = buildTestCase(testCase,niter,noise_level,ifSparPrior,likelihoodType,fixInit,tuneMixing)

if nargin < 7, tuneMixing = 0; end
if nargin < 6, fixInit = 1; end
...
switch testCase
    ...
    case 12 % matrix composition QoI
        ...
        model.sig_eps   = 0.03;
        model.n_snap    = 5;
    case 13 % mean and var of bump sizes
        model.th_true   = [log(0.1), log(4), -1];
        model.th_ind    = [1,2,4];
        model.eqtype    = 'reactdiffuse1d2sp';
        model.n_spec    = 2;
        ...
        model.threshold = 0.5;
        model.n_snap    = 1;
        model.mu_eps    = zeros(4,1);
        model.sig_eps   = [0.096; 0.050; 0.36; 0.63]; % sigmas of [mean1; var1; mean2; var2]
end
...
% define proposal & posterior
if any(tuneMixing)
    if numel(tuneMixing) == 1
        model.sig_q = tuneMixing*ones(size(model.th_true));
    elseif numel(tuneMixing) == numel(model.th_true)
        model.sig_q = reshape(tuneMixing,size(model.th_true));
    else
        error('proposal sigma incompatible!')
    end
    disp(['proposal sigma = ',num2str(model.sig_q)])
end
...
th0             = model.th_true*0;      % pick a starting point
